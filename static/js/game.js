...
function connectSocket(e) {
    ...
    socket.on("msg", function(data) {
        ...
        startGame.innerHTML = `<p id = "serverInfo" class = "text-2xl"> </p>`

        const newServerInfo = document.getElementById("serverInfo")
        newServerInfo.innerHTML = data.msg;
    });
...
socket.on('startgame', function(data) {
    ...
    socket.on("connect", function(){
        ...
        socket.emit(`join`, {username: localStorage.getItem('username'), location: window.location.pathname.split('/')[2]});
    });
...
function update() {
    ...
    ctx.save();
        ...
        if (side.is === 0) {
            ...
            ctx.textAlign = "end";
            ctx.fillText(otherUser.username + ": " + otherUser.points  , canvas.width - 5, 25);
        }
        else {
            ctx.fillText(otherUser.username + ": " + otherUser.points, 5, 25);
            ctx.textAlign = "end";
            ctx.fillText(user.username + " : " + user.points, canvas.width - 5, 25);
        }
    ctx.restore();
